...
contract ArcadeV4CurveTest is Test {
    ...
    // -------------------------------------------------------------------
    // Creator allocations: per-launch curve parameterization (2026-08-19)
    // -------------------------------------------------------------------

    /// (a) allocation 0 params equal the constants EXACTLY (zero-regression).
    function test_alloc_zeroParamsEqualConstants() public pure {
        ArcadeV4Curve.CurveParams memory p = ArcadeV4Curve.paramsForAllocation(0);
        assertEq(p.virtualUsdc, ArcadeV4Curve.VIRTUAL_USDC_RESERVE, "vUsdc");
        assertEq(p.virtualToken, ArcadeV4Curve.VIRTUAL_TOKEN_RESERVE, "vToken");
        assertEq(p.curveSupply, ArcadeV4Curve.CURVE_SUPPLY, "curveSupply");
        assertEq(p.migrationLpTokens, ArcadeV4Curve.MIGRATION_LP_TOKENS, "lpTokens");
        assertEq(p.migrationFee, ArcadeV4Curve.MIGRATION_FEE, "migFee");
        // defaultParams() is the same thing.
        ArcadeV4Curve.CurveParams memory d = ArcadeV4Curve.defaultParams();
        assertEq(d.virtualUsdc, p.virtualUsdc, "default vUsdc");
        assertEq(d.virtualToken, p.virtualToken, "default vToken");
        assertEq(d.curveSupply, p.curveSupply, "default curveSupply");
        assertEq(d.migrationLpTokens, p.migrationLpTokens, "default lpTokens");
        assertEq(d.migrationFee, p.migrationFee, "default migFee");
        // K is preserved: virtualUsdc * virtualToken == K_CONSTANT.
        assertEq(p.virtualUsdc * p.virtualToken, ArcadeV4Curve.K_CONSTANT, "K preserved");
    }

    /// (b) supply conservation: curveSupply' + migrationLpTokens' + A == T at an
    ///     arbitrary allocation (20% here), exactly.
    function test_alloc_supplyConservation() public pure {
        uint256 t = ArcadeV4Curve.TOTAL_SUPPLY;
        uint16 allocBps = 2_000;
        ArcadeV4Curve.CurveParams memory p = ArcadeV4Curve.paramsForAllocation(allocBps);
        uint256 a = (t * allocBps) / 10_000;
        assertEq(p.curveSupply + p.migrationLpTokens + a, t, "curve + LP + alloc == TOTAL_SUPPLY");
        // The market fraction Mt = curveSupply' + migrationLpTokens' = T - A.
        assertEq(p.curveSupply + p.migrationLpTokens, t - a, "market == T - A");
        // Curve keeps the 806/1000 ratio of the market supply.
        assertEq(p.curveSupply, ((t - a) * ArcadeV4Curve.CURVE_SUPPLY) / t, "curve = Mt*806/1000");
    }

    /// (c) Price continuity holds at every allocation level: the AMM seed price
    ///     equals the curve's final marginal price (the SAME exact assertion the
    ///     allocation-0 test uses, parameterized). Because the scaling is a clean
    ///     rational Mt/T and K is preserved, the equality is bit-exact.
    function _reserveAtP(ArcadeV4Curve.CurveParams memory p, uint256 sold) internal pure returns (uint256) {
        uint256 k = p.virtualUsdc * p.virtualToken;
        return k / (p.virtualToken - sold) - p.virtualUsdc;
    }

    function _assertContinuity(uint16 allocBps) internal pure {
        ArcadeV4Curve.CurveParams memory p = ArcadeV4Curve.paramsForAllocation(allocBps);
        uint256 realAtGrad = _reserveAtP(p, p.curveSupply);
        uint256 marginal = ArcadeV4Curve.spotPrice(p, p.curveSupply, realAtGrad);
        uint256 lpUsdc = ArcadeV4Curve.graduationLiquidityUsdc(p, realAtGrad);
        uint256 seedPrice = (lpUsdc * 1e18) / p.migrationLpTokens;
        assertEq(seedPrice, marginal, "AMM opens at curve marginal price (0 cliff) at this alloc");
        // And the open FDV stays ~$60k (60 microUSDC/token * 1B) at every level.
        assertEq(marginal, 60, "graduation marginal ~= $60k FDV at this alloc");
    }

    function test_alloc_continuityAcrossLevels() public pure {
        _assertContinuity(0);
        _assertContinuity(1_000);
        _assertContinuity(5_000);
        _assertContinuity(9_000);
    }
