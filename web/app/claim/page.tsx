...
function ClaimPageInner() {
  ...
  const onClaim = async () => {
    ...
    setSubmitting(true);
    try {
      // NOTE (2026-06-30): the Multicall3From "one signature" fast path is
      // DISABLED. Arc's `callFrom` precompile (0x18..03) that Multicall3From
      // routes through is broken on the current Arc version (codesize 1, every
      // aggregate3 reverts with StackUnderflow), so folding sync + authorize +
      // claim into one signature reverts on-chain. We always use the staged
      // direct-tx flow below: each call runs straight from the user's wallet so
      // msg.sender == recipient naturally and no precompile is involved. With a
      // 0 timelock the claim still settles immediately (authorize sets
      // executeAfter to its block timestamp; the later claim tx satisfies
      // block.timestamp >= executeAfter). Re-enable the batch only once a
      // working sender-preserving multicall exists on Arc.
